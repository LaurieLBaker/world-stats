...
In 1993, the New Zealand Department of Justice surveyed jury pools across the country to examine the representation of Māori, the Indigenous people of New Zealand. The survey covered all potential jurors who arrived at court during a period in September and October 1993. Nationally, Māori made up 9.5% of the eligible population and 10.1% of the jury pool, suggesting that Māori were adequately represented. In this activity, we'll investigate whether this overall conclusion changes when we examine the data by region.

![](plots/actual_maori_jury_pool_map.png){fig-alt="Number of Māori in regional jury district pools surveyed in 1993. Major urban areas like Auckalnd and Rotorua have the highest number of Māori in the jury pool with 74 and 79 Māori respectively." fig-align="center"}
...
-   The **response variable** is whether someone is in the jury pool.
-   The main **explanatory variable** is whether someone is Māori.
...
The table below contains the number of Māori in the jury pool and the total number of people in the jury pool for each New Zealand jury district. These data come from *Trial By Peers* (Dunstan, Paulin, and Atkinson 1995), a nationwide survey of jury pools conducted in 1993.
...
At the national level, does Māori representation appear to match the eligible population?
:::
...
From the reading, can you think of a few reasons why jury pools might be selected unequally?
:::

Calculate the percentage of each jury pool that identified as Māori by filling in the blank
...
district_summary <- district_summary |>
  mutate(
    jury_maori_pct = round(jury_maori_pct, 1)
  )
```
...
Create a bar chart showing the percentage of Māori in the jury pool for each district.
...
Westbrooke (1998) compared these jury pool percentages to the percentage of Māori in the eligible population (ages 20–64) from the 1991 New Zealand Census.

```{webr load district census}
...
We can easily do this and calculate the shortfall by joining our two datasets together
...
```{webr calculate difference between observed and expected}
district_summary <- district_summary |>
  mutate(

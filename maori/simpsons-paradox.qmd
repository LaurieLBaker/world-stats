...
At the national level, does Māori representation in the jury pool appear to be higher, lower, or the same as their percentage of the eligible population?
:::
...
Run the setup chunk below before beginning Part 2. It recreates all objects completed in Part 1, so you do not need to rerun the earlier exercises.
...
district_jury <- tribble(
  ...
  mutate(
    n_non_maori = n_jury_pool - n_maori
  )
```
:::
...
Run the setup chunk below before beginning Part 3. It recreates the completed data and calculations from Parts 1 and 2, then loads the census percentages needed for this section.
...
district_summary <- district_jury |>
  mutate(
    jury_maori_pct = n_maori / n_jury_pool * 100
  )
...
We can use these data to calculate the expected number of individuals and compare this to the number observed by joining our two datasets together.
...
Run the setup chunk below before beginning Part 4. It recreates the completed data and calculations from Parts 1–3.
...
district_summary <- district_jury |>
  left_join(district_census, by = "district") |>
  mutate(
    difference = jury_maori_pct - eligible_maori_pct
  )
```
:::
...
Run the setup chunk below before beginning Part 5. It recreates the completed data and calculations from Parts 1–4.
...
district_summary <- district_jury |>
  ...
  mutate(
    difference = jury_maori_pct - eligible_maori_pct,
    expected_maori = eligible_maori_pct / 100 * n_jury_pool,
    shortfall = expected_maori - n_maori
  )
```
:::
...
district_summary |>
  mutate(
    district = forcats::fct_reorder(district, influence_pct),
    highlight = influence_pct >= 10
  ) |>
  ggplot(aes(
    x = district,
    y = influence_pct,
  ...
  coord_flip() +
  geom_text(
    aes(label = paste0(round(influence_pct, 1), "%")),
    hjust = -0.15,
    size = 3.5
  ) +
  scale_fill_manual(
  ...
  labs(
    title = "How Much Does Each District Contribute?",
    subtitle = "Districts contributing at least 10% of the national jury pool are highlighted.",
    x = NULL,
    y = "Contribution to national jury pool (%)"
  ) +
